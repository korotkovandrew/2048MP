...
DSS_RESULT = DSS.loadMatrix(DSS_PATH)
    ...
    def getRecommendedArticles(self, userID: int, numberOfArticles: int) -> list:
        ...
        for i in range(len(userLikes)):
            if i >= numberOfArticles:
                break
            weights = DSS_RESULT[userLikes]
            # get top weights and their indices in pairs (i, w) in DSS_RESULT if indices are not already in userLikes
